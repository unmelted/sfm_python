...
class Colmap(object) : 
    ...
    def cvt_colmap_model(self, dbcur):
            ...
            cursur = conn.cursor()        
            colms = ['focal_length REAL', 'qw REAL', 'qx REAL', 'qy REAL', 'qz REAL', 'skew REAL', 'tx REAL', 'ty REAL', 'tz REAL']
            ...
            for line in lines : 
                ...
                if line[0] != '#' : 
                    ...
                    skew = float(line[7])
                    print(id, focal, skew)
                    q = ('UPDATE cameras SET (?, ?), (?, ?) WHERE (?) = (?)')
                    cursur.execute(q, ('focal_length', focal, 'skew', skew, 'camera_id', id))
