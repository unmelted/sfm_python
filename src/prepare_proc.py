...
def prepare_job(from_path, root_path, type):
    if not os.path.exists(os.path.join(from_path, defn.pts_file_name)):
        return -106

    to_path = os.path.join(root_path, 'images')
    pts = os.path.join(from_path, defn.pts_file_name)
...
def setup_proj_ini(root_path):
    ...
    fi = os.path.join(from_path, defn.feature_ini)
    shutil.copy(fi, root_path)
    fi = os.path.join(from_path, defn.matcher_ini)
    shutil.copy(fi, root_path)
    fi = os.path.join(from_path, defn.mapper_ini)
    shutil.copy(fi, root_path)

    imgpath = os.path.join(root_path, 'images')
    coldb_path = os.path.join(root_path, defn.colmap_db_name)
    outpath = os.path.join(root_path, 'sparse')

    feature = os.path.join(root_path, defn.feature_ini)
    matcher = os.path.join(root_path, defn.matcher_ini)
    mapper = os.path.join(root_path, defn.mapper_ini)
