...
class View(object):
    ...
    def __init__(self, task_mode, image_path, root_path, scale, feature_path, feature_type='sift'):
        ...
        # list of descriptors obtained from feature extraction
    ...
    def extract_features(self):
        ...
        if self.extraction_mode == 'half':
            half_image = cv2.resize(
                self.image, (self.proc_width, self.proc_height))
            ...
            t_keypoints, self.descriptors = detector.detectAndCompute(
                half_image, None)

            for point in t_keypoints:
                keypoint = cv2.KeyPoint(x=point.pt[0]*2, y=point.pt[1]*2, size=point.size, angle=point.angle,
                                        response=point.response, octave=point.octave, class_id=point.class_id)
            ...
            for i in range(0, 4):
                for point in t_keypoints[i]:
                    pt = cv2.KeyPoint(x=point.pt[0]*2, y=point.pt[1]*2, size=point.size, angle=point.angle,
                                      response=point.response, octave=point.octave, class_id=point.class_id)
                    self.keypoints.append(pt)

                for desc in t_descriptors[i]:
                    self.descriptors = np.append(self.descriptors, desc)
                self.descriptors = self.descriptors.reshape(
                    int(len(self.descriptors)/128), 128)
    ...
    def make_mask(self, rows, cols, position):
        ...
        quad4 = [[cols/2, rows/2], [cols, rows/2],
    ...
    def read_features(self):
        ...
        try:
            features = pickle.load(
                open(os.path.join(self.root_path, 'features', self.name + '.pkl'), "rb"))
            ...
            # convert descriptors into n x 128 numpy array
        ...
        try:
            features = pickle.load(
                open(os.path.join(self.root_path, 'features', self.name + '_mask.pkl'), "rb"))
            ...
            # convert descriptors into n x 128 numpy array
            self.descriptors_mask = np.array(descriptors)

        except FileNotFoundError:
            logging.error(
                "Pkl with mask file not found for image %s. Computing from scratch", self.name)
    ...
    def write_features(self):
        ...
        features_file = open(os.path.join(
            self.root_path, 'features', self.name + '.pkl'), 'wb')
        ...
        if self.bMask > 0:
            ...
            features_file2 = open(os.path.join(
                self.root_path, 'features', self.name + '_mask.pkl'), 'wb')
